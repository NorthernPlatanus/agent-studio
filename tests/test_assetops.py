...
def test_unknown_name_raises_and_lists_what_exists():
    with pytest.raises(UnknownAssetOp) as e:
        resolve_op(_cfg({"reduce_model_poly": "x"}), "reduce_model_ploy")
    assert "reduce_model_poly" in str(e.value)
...
def test_planner_payload_lists_the_available_ops(tmp_path, monkeypatch):
    ...
    cfg._data["asset_ops"] = {"reduce_model_poly": "npx @gltf-transform/cli simplify a b"}
    ...
    assert "reduce_model_poly" in text and "gltf-transform" in text
