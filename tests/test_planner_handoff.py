...
## M3 — Data Layer
- [x] **T-101** Load the schema via SchemaLoader; render it in features/table/ui. {done1} See ADR-0007 for the reader contract.
- [ ] **T-140** Tune ranking on the real dataset. {open1}
- [~] **T-120** Load the compact index behind the query seam. {open2}

## M4 — Core Features
- [!] **T-112** Blocked on the ingest pipeline.
...
def test_references_are_rescued_from_the_dropped_tail(tmp_path):
    ...
    _board(tmp_path, ctx)
    out = planner_mod._backlog_excerpt(ctx, None)
    assert "ADR-0007" in out, "the pointer worth keeping was truncated away"
