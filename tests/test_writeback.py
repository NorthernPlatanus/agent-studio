...
`Agent:` note. Observed: T-131a merged while the board still read
...
### M4 — Core
- [ ] **T-131** Rebuild the run timeline.
...
def test_a_finished_child_annotates_its_parent_without_completing_it(tmp_path):
    ...
    write_back(ctx, "T-131a", {"id": "T-131a"}, "done", "merged @ 0f1e2d3c4b")
    text = _board(ctx)
    assert "- [ ] **T-131**" in text                    # NOT flipped
    assert "T-131a done — merged @ 0f1e2d3c4b" in text
